    ...
    if (email && password) {
      loginUser(userDetails).then(response=>{
        ...
        if(response.message=="login successful"&&response?.token){
          localStorage.setItem("token", JSON.stringify({token:response?.token||null,userId:response?.user?.id||null}));
            ...
            <div>
              ...
              <div className=" mt-1 flex justify-center items-center">
                <input
                  ...
                  className="appearance-none block w-full px-3 py-2 border rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                />
                {!visible?<p  className="ml-[-55px] mr-[12px] cursor-pointer" style={{borderBottom:'1px solid black'}}
                    onClick={() => setVisible(true)}>
                show
                 </p>:
                 <p  className="ml-[-55px] cursor-pointer mr-[14px]" style={{borderBottom:'1px solid black'}}
